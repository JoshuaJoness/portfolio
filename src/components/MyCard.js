		...
		<CardContent>
			...
			<Typography variant="body2" color="textSecondary" component="p" style={{color:'black'}}>
				...
				<div style={{ height:"100px", display:'flex'}}>							   
					{project.projectLinks.map((link, index) => {
						if (link.includes('github')) {
							return <a style={styles.link} href={link} target='_blank' rel="noopener noreferrer">
				...
				<div style={{height:"200px", border:'1px solid #d3d3d3', borderRadius:'6px', backgroundColor:'#eeeeee', paddingBottom:'5%', paddingTop:'2%'}}>
					<center>
						<b style={{fontFamily: "'Baloo 2', cursive", fontSize:'1.7rem'}}>Description</b>
					</center>
					<ul>
						{project.projectDescription.map((description, index) => <li style={styles.description}>{description}</li>)}
					</ul>
				</div>    
			</Typography>
		</CardContent>
